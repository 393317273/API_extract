...
def main():
       json_num = 0
       result_dict = {}
       for m1 in range(len(class_link9.all_class_link)):
              url = class_link9.all_class_link[m1]
              ...
              for x1 in range(len(field_list1)):
                     field_name = re.findall(r'<code><span class="memberNameLink"><a href=".*">(.*?)</a></span></code>',field_list1[x1])
                     #print(field_name[0])
              ...
              try:
                     ...
                     continue
       with open(f"class_result//class_result9.json","w") as f:
