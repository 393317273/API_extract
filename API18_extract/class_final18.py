...
def main():
       json_num = 0
       result_dict = {}
       for m1 in range(len(class_link18.all_class_link)):
              url = class_link18.all_class_link[m1]
              ...
              extend_content = re.findall('<div class="header">(.*?)<section class="class-description" id="class-description">',class_html,re.S)
              extend_list = list()
              p = re.compile(r'<div class="inheritance" title="Inheritance Tree"><a hre.*">(.*?)</a>')
              for m4 in p.findall(str(extend_content)): 
                     extend_list.append(m4)
              #print(extend_list)


              package_content = re.findall('<div class="header">(.*?)<!-- ========== METHOD SUMMARY =========== -->',class_html,re.S)
              ...
              m2 = m2[-1]
              #print(m2)
              p = re.compile(r'<h1 title="Class (.*)" class="title">', re.MULTILINE)
              ...
              #------------------Field Detail:---------------------------------
              content_filed =re.findall('<section class="field-summary" id="field-summary">(.*?)<!-- ======== CONSTRUCTOR SUMMARY ======== -->',class_html,re.S) 
              content1 = str(content_filed)
              field_list1 = list()
              p = re.compile('<div class="col-first even-row-color">(.*?)<div class="col-last even-row-color">',re.S)
              ...
              for x1 in range(len(field_list1)):
                     field_name = re.findall(r'class="member-name-link">(.*?)</a></code></div>',field_list1[x1])
                     #print(field_name[0])
                     Modifier = re.findall(r'<div class="col-first even-row-color"><code>(.*?) <a href=',str(field_list1[x1]))
              ...
              #------------------Method Detail:---------------------------------
              content_methon = re.findall('<h2>Method Details</h2>(.*?)<!-- ========= END OF CLASS DATA ========= -->',class_html,re.S)
              ...
              try:
                     ...
                     continue
       with open(f"class_result//class_result18.json","w") as f:
